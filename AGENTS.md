...
WeChat channel plugin for Codex and Codex, connecting WeChat via the official iLink Bot API.
...
```
plugins/weixin/
├── server.ts          # MCP Server entry (Codex adapter)
...
│   ├── accounts.ts    # Credential storage (~/.Codex/channels/weixin/)
...
├── skills/            # Skill definitions (shared by Codex and Codex)
...
- **Two-layer design**: `src/` is platform-agnostic WeChat communication;
  `server.ts` is the Codex adapter; `server-codex.ts` is the Codex adapter. Keep them separated.
- **No reverse engineering**: Only use the official iLink Bot API.
- **Credentials**: Always `chmod 0600` for account files.
  State dir: `~/.Codex/channels/weixin/` (overridable via `WEIXIN_STATE_DIR`).
...
- Version numbers must stay in sync across 5 files:
  `package.json`, `server.ts`, `server-codex.ts`, `.Codex-plugin/plugin.json`,
  `.codex-plugin/plugin.json`, and root `.Codex-plugin/marketplace.json`
